...
from blog_app.models import Post,Comment,Contact
from django.core.paginator import EmptyPage,Paginator,PageNotAnInteger
from blog_app.forms import CommentForm,ContactForm,PostAddForm
from taggit.models import Tag
from django.contrib.auth.decorators import login_required
from django.http import HttpResponseRedirect,JsonResponse,Http404,HttpResponse
from blog_app.forms import SignUpForm,UserUpdateForm,ProfileUpdateForm,PostUpdateForm,UserAddForm
...
class post_detail_view2(DetailView,):
    model=Post
...
def mail_send_view(request,id):
    ...
    if request.method=='POST':
        ...
        if form.is_valid():
            ...
            send_mail(subject,message,'nixaach@gmail.com',[cd[form.to]],fail_silently=False,)
            sent=True
    else:
        form=EmailSendForm()
...
def add_post(request):
    ...
    if request.method=='POST':
        ...
        if form.is_valid():
            ...
            return HttpResponse("<h1> PLEASE CORRECT YOUR SLUG FIELD</h1>")
    else:
        form=PostAddForm()
...
def post_detail_view2(request,post,year,month,day):
    post=get_object_or_404(Post,slug=post,status='published',publish__year=year,publish__month=month,publish__day=day)
    comments=post.comments.filter(active=True)
    csubmit=False
    is_liked = False
    ...
    return render(request, 'blog/detail_view.html',
                  {'post': post,  'csubmit': csubmit, 'comments': comments, 'is_liked': is_liked,
